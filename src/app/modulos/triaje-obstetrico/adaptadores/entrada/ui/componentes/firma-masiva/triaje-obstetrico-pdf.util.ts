...
export interface TriajeReporteData {
  ...
  EsGestante?: boolean | number | null;
}
...
export async function construirPdfTriaje(
  ...
  const tieneDatosGestion = [c.FUR, c.FPP, c.EdadGestacional].some(
    (valor) => valor !== null && valor !== undefined && valor !== '',
  );
  if (tieneDatosGestion) {
    seccion('DATOS DE GESTACIÓN');
    dibujarFila(
      [
        { span: 1, text: 'GESTANTE', kind: 'label' },
        {
          span: 1,
          text: c.EsGestante ? 'SÍ' : 'NO',
          kind: 'value',
          align: 'center',
        },
    ...
    dibujarFila(
      [
        { span: 2, text: 'N° CONTROLES PRENATALES', kind: 'label' },
        {
          span: 2,
          text: v(c.NroControlesPrenatales),
          kind: 'value',
          align: 'center',
        },
        { span: 2, text: 'MOVIMIENTOS FETALES', kind: 'label' },
        {
          span: 2,
          text: v(c.MovimientosFetales),
          kind: 'value',
          align: 'center',
        },
      ],
